...
def favicon():
    """Serve a small inline SVG as favicon to avoid 404 noisy requests when no favicon file exists."""
    svg = (
        '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64">'
        '<rect fill="#7B61FF" width="64" height="64"/>'
        '<text x="50%" y="50%" font-size="36" text-anchor="middle" dominant-baseline="central" fill="#fff">Q</text>'
        '</svg>'
    )
...
def upload_file():
    ...
    try:
        # Helpful debug logging for client-side 400s
        print(f"📥 /upload called. request.files keys: {list(request.files.keys())}; request.form keys: {list(request.form.keys())}")
        ...
        # If the uploaded FileStorage has an empty filename (e.g. blobs), try to assign a safe default
        ...
        if filename_raw == '':
            ...
            if mimetype.startswith('audio/'): 
                # prefer wav for recordings
                ext = 'wav'
            elif mimetype.startswith('video/'):
                ext = 'mp4'
            filename = secure_filename(f'upload.{ext}')
        else:
            filename = secure_filename(filename_raw)
        
        # If the extension is not allowed, try to still accept common audio/video mimetypes
        if not allowed_file(filename):
            # allow if mimetype indicates audio/video even if extension missing/unusual
            mimetype = (file.mimetype or '').lower()
